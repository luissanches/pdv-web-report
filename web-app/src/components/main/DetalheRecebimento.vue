<template>
  <div class="auto scroll">
    <div class="row justify-center margin-top30">
      <span class="label bg-secondary text-white shadow-1">Detalhes de Recebimentos</span>
    </div>
          ...
          <table class="q-table responsive">
            ...
            <tbody>
              <tr v-for="sale in sales">
                <td>{{sale.datarecebimento | formatDate}}</td>
                <td>{{sale.nomepagamento}}</td>
                <td>{{sale.valortotalrecebimento | formatCurrency}}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  methods: {
    onSearched(dateStart, dateEnd) {
      let url = `reports/getreceiveammount/${dateStart}/${dateEnd}`
      this.$http.get(url).then(response => {
        ...
        for (let sale of this.$data.sales) {
          totalrecebimento += parseInt(sale.valortotalrecebimento)
        }
